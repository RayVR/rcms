...
//! (`rcms::opt::matshaper`, lcms2 `OptimizeMatrixShaper` / `MatShaperEval16`).
//!
...
//! here, so for those rcms's `Lcms2Compat` falls back to the accurate eval.
//!
//! We assert:
    ...
    for (an, bn) in pairs() {
        ...
        for &out_fmt in &[type_rgb_8(), type_rgb_16()] {
            ...
            let ok = if out_fmt == type_rgb_8() {
                rcms_oracle::do_transform_packed_default_8(
                    &[&ab, &bb],
                    &intents_raw,
                    &bpc,
                    &adapt,
                ...
                rcms_oracle::do_transform_packed_default_16(
                    &[&ab, &bb],
                    &intents_raw,
                    &bpc,
                    &adapt,
            ...
            .unwrap_or_else(|e| panic!("rcms Lcms2Compat build {an} -> {bn}: {e:?}"));
...
fn accurate_matches_lcms2_nooptimize_8bit() {
    ...
    for (an, bn) in pairs() {
        ...
        // rcms Accurate (default): the optimizer never fires.
        let xform =
...
fn divergence_accurate_vs_lcms2compat_report() {
    ...
    // run BOTH rcms-Accurate and rcms-Lcms2Compat over the full 8-bit RGB cube
    ...
    for (an, bn) in pairs() {
        ...
        rcms_oracle::do_transform_packed(
            &[&ab, &bb],
            &intents_raw,
            &bpc,
            &adapt,
